...
export default function CuisineEtape1Screen({ navigation}) {
 ...
 for (let x=0 ; x < recette.length; x++) {
  ...
  fetch(`http://192.168.1.15:3000/menuTer/miseenoeuvre?recettesList=[${titleList}]`)
    .then((response) => response.json())
    .then((data) => {
      for (let x=0 ; x < data.steps.assemblage.length; x++) {
      //console.log(data.steps.assemblage[x].step.target[0]);
      const Recipe = {
    ...
    <ScrollView style={styles.container}>
      <Menu  />
         <View style={styles.generalContainer}>
             ...
             <Text style={styles.title}>Etape 3 : Assemblage </Text>
        {renderSteps}
